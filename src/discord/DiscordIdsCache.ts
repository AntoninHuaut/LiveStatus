...
export class DiscordIdsCache {
    ...
    public get(discordChannelId: string, twitchUserName: string): DiscordIdsCacheModel {
        const item = localStorage.getItem(this.getKey(discordChannelId, twitchUserName));
        return item ? JSON.parse(item) : { messageId: '', eventId: '' };
    }

    public set(discordChannelId: string, twitchUserName: string, idItem: DiscordIdsCacheModel) {
        localStorage.setItem(this.getKey(discordChannelId, twitchUserName), JSON.stringify(idItem));
    }
