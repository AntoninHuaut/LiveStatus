...
export default class GlobalRunnable {
  ...
  public constructor(config: Config) {
    ...
    this.checkIntervalMs = Math.max(
      config.twitch.checkIntervalMs,
      GlobalRunnable.MIN_CHECK_INTERVAL,
    );
  ...
  private async tick() {
    Logger.info(
      `GlobalRunnable (${this.discordClients.length} discordClients - ${this.twitchRuns.length} twitchRuns) ticking`,
    );

    const promises: Promise<void>[] = [];
    this.twitchRuns.forEach((twitchRun: TwitchRunnable) =>
      promises.push(twitchRun.tick())
    );
    await Promise.all(promises);

    promises.length = 0;
    this.discordClients.forEach((discordClient: DiscordClient) =>
      promises.push(discordClient.tick())
    );
    await Promise.all(promises);
  ...
  private createTwitchRunnable(config: Config) {
    ...
    config.discord.discords.map((discord: DiscordData) =>
      twitchChannelsName.add(discord.twitchChannelName)
    );

    twitchChannelsName.forEach((twitchUsername: string) =>
      this.twitchRuns.push(new TwitchRunnable(twitchRequest, twitchUsername))
    );
  }
  ...
  private createDiscordClients(discordConfig: DiscordConfig) {
    ...
    discordConfig.discords.forEach((discord: DiscordData) =>
      this.discordClients.push(
        new DiscordClient(discordRequests, discord, this.checkIntervalMs),
      )
    );
  }
}
