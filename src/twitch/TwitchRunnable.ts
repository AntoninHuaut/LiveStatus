    ...
    try {
      ...
      if (dataArray.length) {
        ...
        if (dataLive.type === "live") {
          ...
          try {
            liveModel.streamImageUrlBase64 = await this.fetchBlobImg(
              liveModel.streamImageUrl,
            );
          } catch (err) {
            Logger.error(
              `[LiveModel::streamImageUrl] Error parsing streamImage url: "${liveModel.streamImageUrl}" error:\n${err.stack}`,
            );
          }
      ...
      Logger.error(
        `TwitchRunnable ${this.twitchUsername} error:\n${err.stack}`,
      );
    }
  }
